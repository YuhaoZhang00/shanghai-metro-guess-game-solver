...
class TestMetroGameSolver:
    ...
    def test_reset_solver(self, solver):
        ...
        result = solver.apply_constraint("航头", "every", "every", 0)
    ...
    def test_apply_constraint_invalid_station(self, solver):
        ...
        result = solver.apply_constraint("不存在的站", "every", "every", 0)
    ...
    def test_apply_multiple_constraints(self, solver):
        ...
        # First constraint
        result1 = solver.apply_constraint("航头", "every", "every", 0)
        assert result1 is not None
        count1 = len(result1)
        
        # Second constraint that should further filter
        result2 = solver.apply_constraint("徐家汇", "none", "none", 1)
    ...
    def test_get_station_summary(self, solver):
        ...
        assert "(" in summary and ")" in summary  # Format: name (lines | districts | year)
    ...
    def test_constraint_consistency(self, solver):
        ...
        result = solver.apply_constraint("人民广场", "some", "none", -1)
        assert result is not None
    ...
    def test_no_match_constraint(self, solver):
        ...
        solver.apply_constraint("航头", "every", "every", 0)
        
        # Apply another constraint that conflicts
        result = solver.apply_constraint("徐家汇", "some", "none", 1)
...
class TestParseInputLine:
    ...
    def test_valid_input(self, solver):
        ...
        result = solver.parse_input_line("航头 every every 0")
        assert result == ("航头", "every", "every", 0)
        
        # Test different constraint values
        result = solver.parse_input_line("徐家汇 some none -1")
        assert result == ("徐家汇", "some", "none", -1)
        
        result = solver.parse_input_line("人民广场 none some 1")
        assert result == ("人民广场", "none", "some", 1)

    def test_invalid_input_format(self, solver):
        """Test parsing invalid input formats."""
        # Too few arguments
        result = solver.parse_input_line("航头 every every")
        assert result is None
        
        # Too many arguments
        result = solver.parse_input_line("航头 every every 0 extra")
        assert result is None
    ...
    def test_invalid_constraint_values(self, solver):
        ...
        result = solver.parse_input_line("航头 invalid every 0")
        assert result is None
        
        # Invalid line match
        result = solver.parse_input_line("航头 every invalid 0")
        assert result is None
        
        # Invalid year match
        result = solver.parse_input_line("航头 every every 2")
        assert result is None
        
        result = solver.parse_input_line("航头 every every -2")
        assert result is None
        
        result = solver.parse_input_line("航头 every every abc")
        assert result is None
...
class TestIntegrationScenarios:
    ...
    def test_typical_solving_scenario(self, solver):
        ...
        result1 = solver.apply_constraint("人民广场", "some", "some", 0)
        assert result1 is not None
        count1 = len(result1)
    ...
    def test_progressive_filtering(self, solver):
        ...
        solver.apply_constraint("龙阳路", "none", "some", -1)
        counts.append(len(solver.possible_stations))
    ...
    def test_solver_with_reset(self, solver):
        ...
        solver.apply_constraint("航头", "every", "every", 0)
        solver.apply_constraint("徐家汇", "some", "none", 1)
        ...
        result = solver.apply_constraint("龙阳路", "none", "some", 1)
        assert result is not None
        assert len(solver.guess_history) == 1